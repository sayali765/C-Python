...
def match_job_role(resume_keywords):
    """
    Match resume keywords with predefined job keywords and calculate the score.

    Args:
    - resume_keywords (list of str): Keywords extracted from the resume.

    Returns:
    - tuple: Matching job role and its score if found, None otherwise.
    """
    max_score = 0
    matching_job_role = None
    for job_role, job_role_keywords in job_keywords.items():
        score = calculate_score(resume_keywords, job_role_keywords)
        if score > max_score:
            max_score = score
            matching_job_role = job_role
    return matching_job_role, max_score

def process_resumes_in_directory(directory_path):
    """
    Process all resumes in a directory and determine the matching job role.
    ...
    for filename in os.listdir(directory_path):
        if filename.endswith(".pdf"):
            ...
            matching_job_role, score = match_job_role(resume_keywords)
            if matching_job_role:
                print(f"Resume '{filename}' matches the job role: {matching_job_role} (Score: {score:.2f}/5)")
            else:
                print(f"No matching job role found for resume '{filename}'")
